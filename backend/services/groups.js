...
export const create = async (userId, workspaceId, groupName) => {
  try {
    ...
    const res = await db.query(
      `INSERT INTO groups (name, workspace_id) VALUES ($1, $2) RETURNING *`,
      [finalGroupName, workspaceId]
    );

    // Update the count of groups created
    await db.query(`UPDATE workspaces SET groups_created = $1 WHERE id = $2`, [
      found.groups_created + 1,
      workspaceId,
    ]);
...
export const createMany = async (userId, workspaceId, numGroups) => {
  try {
    ...
    const [res, _] = await Promise.all([
      ...
      db.query(
        `update workspaces set groups_created = groups_created + $1 where id = $2`,
...
export const join = async (userId, groupId) => {
  try {
    ...
    if (!data.workspace_id)
      return {
        error:
          "Cannot join group: User is not in the same workspace as the group",
        status: 400,
      };
    ...
    if (data.group_joined)
      return {
        error:
          "Cannot join group: User is already in a group in this workspace",
        status: 400,
      };

    // Check membership limit
    const members = (
      await db.query(`SELECT count(*) FROM memberships WHERE group_id = $1`, [
        groupId,
      ])
    ).rows[0];
...
export const addUser = async (userId, targetId, groupId) => {
  try {
    ...
    if (!data.workspace_id)
      return {
        error:
          "Cannot join group: Target is not in the same workspace as the group",
        status: 400,
      };
    // Check that the user is a student
    ...
    if (data.target_cur_group)
      return {
        error:
          "Cannot join group: Target is already in a group in this workspace",
        status: 400,
      };
...
export const deleteGroup = async (userId, groupId) => {
  try {
    ...
    // Delete the group
    await db.query(`DELETE FROM groups WHERE id = $1 RETURNING *`, [groupId]);
