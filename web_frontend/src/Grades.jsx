...
const Grades = () => {
  ...
  const fetchAssignments = async () => {
    try {
      ...
      if (response.status === 200) {
        ...
        enqueueSnackbar(`Failed to fetch assignments: ${response.message}`, {
          variant: "error",
        });
      }
    } catch (error) {
  ...
  const fetchAverages = async () => {
    try {
      const response = await Api.Assignments.GetAveragesByAssignment(
        selectedAssignment,
        page,
        perPage,
        userId
      ...
      if (response.status === 200) {
        ...
        enqueueSnackbar(`Failed to fetch averages: ${response.message}`, {
          variant: "error",
        });
      }
    } catch (error) {
  ...
  const fetchCompletion = async () => {
    try {
      const response = await Api.Assignments.GetCompletionByAssignment(
        selectedAssignment,
        page,
        perPage,
        userId
      ...
      if (response.status === 200) {
        ...
        enqueueSnackbar(
          `Failed to fetch completion data: ${response.message}`,
          { variant: "error" }
        );
      }
    } catch (error) {
  ...
  return (
    <div className="grades-page-container">
      ...
      <div className="assignment-selector-container">
        <label
          className="assignment-selector-label"
      ...
      <div className="view-slider">
        <button className="btn-primary-custom" onClick={toggleView}>
          {view === "average"
            ? "Switch to Completion Status"
            : "Switch to Average Ratings"}
        </button>
      </div>
