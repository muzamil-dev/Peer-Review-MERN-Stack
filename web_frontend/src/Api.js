...
export default {
  ...
  Groups: {
    ...
    DeleteGroup: async (userId, groupId) => {
      ...
      const response = await apiRequest(DELETE, getUrl(GROUPS, groupId), {
        data: payload,
      });
      return {
        status: response.status,
  ...
  Assignments: {
    ...
    GetAllReviewsAboutTarget: async (assignmentId, targetId) => {
      const response = await apiRequest(
        GET,
        getUrl(ASSIGNMENTS, `${assignmentId}/target/${targetId}`),
        null
      );
      return {
        status: response.status,
    ...
    GetAveragesByAssignment: async (assignmentId, page, perPage, userId) => {
      ...
      const response = await apiRequest(
        GET,
        getUrl(ASSIGNMENTS, `${assignmentId}/averages${queryParams}`),
        { userId }
      );
      return {
        status: response.status,
    ...
    GetCompletionByAssignment: async (assignmentId, page, perPage, userId) => {
      ...
      const response = await apiRequest(
        GET,
        getUrl(ASSIGNMENTS, `${assignmentId}/completion${queryParams}`),
        { userId }
      );
      return {
        status: response.status,
  ...
  Workspaces: {
    ...
    GetGroups: async (workspaceId) => {
      ...
      const response = await axios.get(
        getUrl(WORKSPACES, `${workspaceId}/groups?members=true`),
        config
      );
      return {
        status: response.status,
     ...
     * @returns {Promise<{ status: number, data: object[], message: string }>} The list of journals
     */
    GetUserJournalsByWorkspaceAdmin: async (
      workspaceId,
      userId,
      requesterId
    ) => {
    ...
    getWeeks: async (workspaceId) => {
      try {
        const config = getConfig(); // Ensure the token is included in the headers
        const response = await axios.get(
          getUrl(WORKSPACES, `${workspaceId}/weeks`),
          config
        );
        return {
          status: response.status,
