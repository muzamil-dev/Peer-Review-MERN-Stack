...
const getUrl = (prefix, route) => {
  if (process.env.NODE_ENV === "production") {
    ...
    return "http://localhost:5001/" + prefix + route;
  }
...
let refreshSubscribers = []; // List of subscribers (requests) to retry once the refresh is done
...
const refreshAccessToken = async () => {
  try {
    ...
    console.error(
      "Api.js: Error Caught: Failed to refresh access token:",
      error
    );
    return null;
  }
...
// Function to add subscribers to the list for retrying the request
  ...
  async (config) => {
    ...
    if (token) {
      ...
      if (exp < now) {
        ...
        // If refresh is in progress, wait for the new token and retry the request
      ...
      // Set the token in the request header if it is still valid
...
export default {
  ...
  Assignments: {
    ...
    GetAssignmentInfo: async (assignmentId) => {
      const response = await apiRequest(
        GET,
        getUrl(ASSIGNMENTS, assignmentId),
        null
      );
      return {
        status: response.status,
    ...
    GetAllReviewsByUser: async (assignmentId, userId) => {
      ...
      if (!userId) {
        response = await apiRequest(
          GET,
          getUrl(ASSIGNMENTS, `${assignmentId}/user`),
          null
        );
      } else {
        response = await apiRequest(
          GET,
          getUrl(ASSIGNMENTS, `${assignmentId}/user/${userId}`),
          null
        );
      }
      return {
    ...
    GetAllReviewsAboutTarget: async (assignmentId, targetId) => {
      const response = await apiRequest(
        GET,
        getUrl(ASSIGNMENTS, `/averages/${assignmentId}/target/${targetId}`),
        null
      );
      return {
        status: response.status,
  ...
  Users: {
    ...
    RequestPasswordReset: async (email) => {
      ...
      const response = await apiRequest(
        POST,
        getUrl(USERS, "requestPasswordReset"),
        payload
      ).catch((err) => {
        console.error(err);
        return err.response || Response503;
  ...
  Workspaces: {
    ...
    GetAssignments: async (workspaceId) => {
      const response = await apiRequest(
        GET,
        getUrl(WORKSPACES, `${workspaceId}/assignments`),
        null
      );
      return {
        status: response.status,
    ...
    GetGroups: async (workspaceId) => {
      const response = await apiRequest(
        GET,
        getUrl(WORKSPACES, `${workspaceId}/groups`),
        null
      );
      return {
        status: response.status,
    ...
    RemoveActiveInvite: async (userId, workspaceId) => {
      ...
      const response = await apiRequest(
        DELETE,
        getUrl(WORKSPACES, `${workspaceId}/removeInvite`),
        payload
      );
      return {
        status: response.status,
    ...
    GetAllStudents: async (workspaceId) => {
      const response = await apiRequest(
        GET,
        getUrl(WORKSPACES, `${workspaceId}/students`),
        null
      );
      return {
        status: response.status,
    ...
    GetStudentsWithoutGroup: async (workspaceId) => {
      const response = await apiRequest(
        GET,
        getUrl(WORKSPACES, `${workspaceId}/ungrouped`),
        null
      );
      return {
        status: response.status,
  ...
  Analytics: {
    ...
    GetAnalyticsForUser: async (targetId, workspaceId, userId) => {
      ...
      const response = await apiRequest(
        GET,
        getUrl(ANALYTICS, `workspace/${workspaceId}/user/${targetId}`),
        payload
      );
      return {
        status: response.status,
     ...
     * }>}
     */
    GetAnalyticsForAssignment: async (
      assignmentId,
      userId,
      page = 0,
      perPage = 10
      ...
      const response = await apiRequest(
        ...
        getUrl(
          ANALYTICS,
          `assignment/${assignmentId}?page=${page}&perPage=${perPage}`
        ),
        payload
      );
      return {
