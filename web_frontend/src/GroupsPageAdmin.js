...
const GroupsPageAdmin = () => {
  ...
  const fetchWorkspaceDetails = async () => {
    const token = localStorage.getItem("accessToken");
    const response = await Api.Workspaces.GetWorkspaceDetails(
      workspaceId,
      token
    );
        ...
        setGroups(
          response.data.groups.map((group) => ({
            ...
            members: Array.isArray(group.members)
              ? group.members.filter((member) => member && member.userId)
              : [],
          }))
        );
      } else {
  ...
  const handleInsertUserSubmit = async (e) => {
    ...
    try {
      const groupId =
        insertUserData.role === "Instructor" ? null : insertUserData.groupId;
      ...
      if (response.status === 201) {
        ...
        enqueueSnackbar(`Failed to insert user: ${response.message}`, {
          variant: "error",
        });
      }
    } catch (error) {
      console.error("Error inserting user into workspace:", error);
      enqueueSnackbar("Error inserting user into workspace", {
        variant: "error",
      });
    }
  };
  ...
  const handleAddUserToGroup = async (targetId, groupId) => {
    ...
    const response = await Api.Groups.MoveUser(
      currentUserId,
      targetId,
      workspaceId,
      groupId
        ...
        prevGroups.map((group) => {
          if (group.groupId === groupId) {
            return {
              ...
              members: [
                ...group.members,
                ungroupedMembers.find((member) => member.userId === targetId),
              ],
            };
          }
          return group;
  ...
  const handleKickFromGroup = async (targetId) => {
    ...
    if (response.success) {
      setSelectedGroupMembers((prevMembers) =>
        prevMembers.filter((member) => member.userId !== targetId)
      );
      setGroups((prevGroups) =>
        prevGroups.map((group) => {
      ...
      setUngroupedMembers((prevMembers) => [
        ...prevMembers,
        selectedGroupMembers.find((member) => member.userId === targetId),
      ]);
      setSelectedMemberGroup((prevState) => {
  ...
  const handleKickFromWorkspace = async (targetId) => {
    ...
    const response = await Api.Workspaces.RemoveUser(
      currentUserId,
      targetId,
      workspaceId,
      token
  ...
  const confirmDeleteGroup = async () => {
    ...
    const response = await Api.Groups.DeleteGroup(
      currentUserId,
      groupToDelete,
      token
    );
  ...
  const handleSubmit = async (event) => {
    ...
    const response = await Api.Workspaces.EditWorkspace(
      currentUserId,
      workspaceId,
      formData.name
    );
  ...
  const handleGroupChange = async (memberId, newGroupId) => {
    ...
    if (oldGroupId) {
      ...
      if (!removeResponse.success) {
        console.error(
          `Failed to remove user ${memberId} from group ${oldGroupId}`
        );
      }
    }
  ...
  const handleImportCSV = async () => {
    ...
    if (!currentUserId || !csvFile) {
      enqueueSnackbar("Please select a file and ensure you are logged in.", {
        variant: "error",
      });
      return;
    }
    ...
    try {
      const response = await Api.Workspaces.importCSV(
        currentUserId,
        workspaceId,
        csvFile
      );
      if (response.success) {
        ...
        enqueueSnackbar("Failed to import CSV: " + response.message, {
          variant: "error",
        });
      }
    } catch (error) {
      console.error("Error importing CSV:", error);
      enqueueSnackbar("Error importing CSV: " + error.message, {
        variant: "error",
      });
    }
  };
  ...
  const handleCreateJournals = async () => {
    try {
      ...
      const response = await Api.Workspaces.createJournals(
        workspaceId,
        payload
      );

      if (response.status === 201) {
        enqueueSnackbar("Journals created successfully!", {
          variant: "success",
        });
        checkForJournals(); // Refresh the journals status
        handleCloseJournalModal(); // Close the modal
      } else {
        enqueueSnackbar(`Failed to create journals: ${response.message}`, {
          variant: "error",
        });
      }
    } catch (error) {
    ...
    <div className={styles.workspaceAdmin}>
      <div className={styles.sidebarContainer}>
        ...
        <nav className={`${styles.sidebar} ${sidebarOpen ? "open" : ""}`}>
          <a className={styles.navbarBrand} href="/DashboardPage">
            Rate My Peer
          </a>
          <ul className={styles.navLinks}>
            ...
            <li>
              <input
                className={`mb-3 ${styles.inputCSV}`}
                type="file"
                accept=".csv"
            ...
            <li>
              <button className="btn btn-light" onClick={openForm}>
                Edit Workspace
              </button>
            </li>
            <li>
            ...
            <li>
              <button
                ...
                onClick={handleOpenInsertUserModal}
              >
                Insert User
              </button>
            </li>
            <li>
              {hasJournals ? (
                <button
                  className="btn btn-primary"
                  onClick={() =>
                    navigate(`/workspaces/${workspaceId}/admin/journals`)
                  }
                >
                  View Journals
                </button>
          ...
          <div className={styles.modalContent}>
            ...
            <form onSubmit={handleInsertUserSubmit}>
              ...
              <div className={styles.modalActions}>
                <button type="submit" className={`btn mr-2 ${styles.custBtn}`}>
                  Insert User
                </button>
                <button
                  type="button"
                  className={`btn ${styles.custBtnDark}`}
                  onClick={handleCloseInsertUserModal}
                >
                  Cancel
                </button>
              </div>
        ...
        <div className={styles.modalOverlay}>
          <div className={styles.modalContent}>
            <form onSubmit={handleSubmit} className={styles.formContainer}>
              ...
              <button type="submit" className={`btn mb-2 ${styles.custBtn}`}>
                Save
              </button>
              <button
                type="button"
                  ...
                  <div key={member.userId} className={styles.memberEditRow}>
                    <div className="d-flex justify-content-between">
                      ...
                      <select
                        ...
                        onChange={(e) =>
                          handleGroupChange(member.userId, e.target.value)
                        }
                    ...
                    <div className="btn-group mb-3 d-flex justify-content-center">
                      <button
                        ...
                        onClick={() =>
                          handleKickConfirmation(member.userId, "group")
                        }
                      >
                        Kick from Group
                      </button>
                      <button
                        ...
                        className="btn btn-outline-dark mt-2"
                        onClick={() =>
                          handleKickConfirmation(member.userId, "workspace")
                        }
                      >
                        Kick from Workspace
                      </button>
      ...
      <div className={styles.contentContainer}>
                      ...
                      <div className="card-body d-flex flex-column">
                        ...
                        <div className="mt-auto">
                          <button
                            className={`btn mr-2 ${styles.custBtn}`}
                            onClick={() =>
                              handleOpenEditForm(group.groupId, group.members)
                            }
                          >
                            Edit
                          </button>
        ...
        <div
          className={`modal fade ${showKickConfirmModal ? "show d-block" : ""}`}
          tabIndex="-1"
          role="dialog"
          style={{ backgroundColor: "rgba(0,0,0,0.5)" }}
        >
            ...
            <div className="modal-content">
              <div className="modal-header">
                ...
                <button
                  type="button"
                  className="close"
                  onClick={() => setShowKickConfirmModal(false)}
                  aria-label="Close"
                >
                  <span aria-hidden="true">&times;</span>
                </button>
