...
const AdminJournalsPage = () => {
  ...
  const fetchStudents = async () => {
    try {
      ...
      if (response.status === 200) {
        ...
        const students = groups.reduce((acc, group) => {
  ...
  const fetchJournals = async (studentId) => {
    try {
      const response = await Api.Workspaces.GetUserJournalsByWorkspaceAdmin(
        workspaceId,
        studentId
      );
      ...
      if (response.status === 200 && response.data) {
        setJournals(journals); // Ensure that journals is always an array
      } else {
            ...
            {journals.map((journal, index) => (
              <li
                ...
                className={`journal-entry ${
                  journal.submittedAt ? "" : "not-submitted"
                }`}
        ...
        <div className="popup-overlay" onClick={() => setIsPopupOpen(false)}>
          <div className="popup-content" onClick={(e) => e.stopPropagation()}>
            ...
            <p>
              ...
              {selectedJournal.submittedAt
                ? new Date(selectedJournal.submittedAt).toLocaleDateString()
                : "Not submitted"}
            </p>
            <div
              className="formatted-content"
              dangerouslySetInnerHTML={formatContent(selectedJournal.content)}
            />
